...
CudaDims CudaTwoDim(size_t size_x, size_t size_y) {
   ...
   */
  CudaDims dim;
  size_t num_blocks_x = (size_x + 16 - 1) / 16;
  size_t num_blocks_y = (size_y + 16 - 1) / 16;
...
// helper function for Compact and SetItem functions
__device__ uint32_t compact_idx(uint32_t gid, CudaVec shape, CudaVec strides) {
  uint32_t flat_idx = 0;
  for (int i = 0; i < shape.size; i++) {
    uint32_t c_stride = 1;
    for (int j = i+1; j < shape.size; j++) { c_stride *= shape.data[j]; }
    //cout << gid / c_stride << " ";
    flat_idx += (gid / c_stride) * strides.data[i];
    gid %= c_stride; 
  }
  return flat_idx;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  if (gid < size) {
    uint32_t flat_idx = compact_idx(gid, shape, strides);
    out[gid] = a[offset + flat_idx];
  }
  /// END YOUR SOLUTION
}
...
__global__ void MatMulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M,
  ...
  if (i < M && j < P) {
    scalar_t acc = 0;
    for (int k = 0; k < N; k++) { acc += a[N*i + k] * b[P*k + j]; }
    out[P*i + j] = acc;
  }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  std::cout << dim.block.x << " " << dim.block.y << " " << dim.block.z << " " << std::endl;
  std::cout << dim.grid.x << " " << dim.grid.y << " " << dim.grid.z << " " << std::endl;
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t cur_max = a[gid * reduce_size];
    for (int i = 0; i < reduce_size; i++) {
      if (a[gid * reduce_size + i] > cur_max) { cur_max = a[gid * reduce_size + i]; }
    }
    out[gid] = cur_max;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
    scalar_t acc = 0;
    for (int i = 0; i < reduce_size; i++) { acc += a[gid * reduce_size + i]; }
    out[gid] = acc;
  }
}

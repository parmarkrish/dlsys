...
def parse_mnist(image_filename, label_filename):
    ...
    """
    ### BEGIN YOUR CODE
    # should use struct, gzip and numpy
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    y_b = y.reshape(num_batches, -1)
    for i in range(num_batches):
      Z_I = np.exp(X_b[i].dot(theta))
      Z_I /= np.sum(Z_I, axis=1)[:, np.newaxis] # normalize
      Z_I[np.arange(batch_size), y_b[i]] -= 1 # subtract I
      theta_grad = X_b[i].T.dot(Z_I)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    y_b = y.reshape(num_batches, -1)
    for i in range(num_batches):
      Z1 = np.maximum(X_b[i].dot(W1), 0)

      G2 = np.exp(Z1.dot(W2))
      G2 /= np.sum(G2, axis=1)[:, np.newaxis] # normalize
      G2[np.arange(batch_size), y_b[i]] -= 1 # subtract I

      G1 = (Z1 > 0) * G2.dot(W2.T)

      grad_W1 = 1/batch_size * X_b[i].T.dot(G1)
      grad_W2 = 1/batch_size * Z1.T.dot(G2)

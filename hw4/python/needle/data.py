...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        self.X, self.y = np.array([], dtype=np.ubyte).reshape(0, 3072), []
        for i in range(1,6):
          data_path = f"data_batch_{i}" if train else "test_batch"
          with open(os.path.join(base_folder, data_path), 'rb') as fo:
            data_dict = pickle.load(fo, encoding='bytes')
          X_batch, y_batch = data_dict[b'data'], data_dict[b'labels']
          self.X = np.vstack([self.X, X_batch])
          self.y.extend(y_batch)
          if not train: break
        self.X = self.X.reshape(-1, 3, 32, 32) / 255.
        self.y = np.array(self.y) # convert into numpy array as it is necessary for multi-indexing

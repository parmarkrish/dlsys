...
class TensorTuple(Value):
    ...
    def __getitem__(self, index: int):
        print('in __getitem__ with index: ', index)
...
class Tensor(Value):
    ...
    def sum(self, axes=None, keepdims=False):
        return needle.ops.Summation(axes, keepdims=keepdims)(self)
...
def print_node(node):
  print('--Node info--')
  print('id: ', str(id(node))[-4:])
  print('inputs: ', [str(id(x))[-4:] for x in node.inputs])
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    print('len reverse_topo_order: ', len(reverse_topo_order)) # debug

    ### BEGIN YOUR SOLUTION
    for node in reverse_topo_order:
      node.grad = sum_node_list(node_to_output_grads_list[node])
      #print_node(node) # debug
      if node.op:
        in_grads = node.op.gradient(node.grad, node)
        in_grads = in_grads if type(in_grads) is tuple else (in_grads,) # make in_grads tuple if not already
        #print('in_grads: ')
        #print_node(in_grads)
        for in_node, in_grad in zip(node.inputs, in_grads):
          print('inside for loop')
          #print('in_node')
          #print_node(in_node)
          print('in_grad')
          print_node(in_grad)
          node_to_output_grads_list.setdefault(in_node, []).append(in_grad)
    ### END YOUR SOLUTION

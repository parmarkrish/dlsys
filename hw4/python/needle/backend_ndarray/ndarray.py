...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self.shape) != prod(new_shape) or not self.is_compact():
            print(prod(self.shape))
            print(prod(new_shape))
            print(self.is_compact())
            raise ValueError
        
        return self.make(new_shape, strides=None, device=self.device, handle=self._handle)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape_strides = [tuple(s[a] for a in new_axes) for s in (self.shape, self.strides)]

        return self.make(new_shape_strides[0], strides=new_shape_strides[1], 
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if self.shape[i] != 1 and new_shape[i] != self.shape[i]:
                raise Exception("Shapes are not broadcastable")
        
        new_strides = tuple(0 if sh == 1 else st for sh, st in zip(self.shape, self.strides))

        return self.make(new_shape, strides=new_strides, device=self.device,
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape, new_stride, new_offset = [], [], self._offset

        for i, s in enumerate(idxs):
            new_shape.append(1 + (s.stop - s.start - 1) // s.step)
            new_stride.append(self.strides[i] * s.step)
            new_offset += self.strides[i] * s.start


        return self.make(new_shape, strides=tuple(new_stride), device=self.device, 
    ...
    def stack(arrays, axis):
      ...
      (new_shape := list(arrays[0].shape)).insert(axis, len(arrays))
      stacked_arr = NDArray.make(new_shape, device=arrays[0].device)
      for i in range(len(arrays)):
        indexes = tuple(i if j == axis else slice(None) for j in range(len(new_shape)))
        stacked_arr[indexes] = arrays[i]
      return stacked_arr
    
    def split(self, axis):
      # added implementation of split
      split_arrays, split_size = [], self.shape[axis]
      for i in range(split_size):
        indexes = tuple(i if j == axis else slice(None) for j in range(self.ndim))
        new_shape = tuple(s for i, s in enumerate(self.shape) if i != axis)
        split_arrays.append(self[indexes].compact().reshape(new_shape))
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_strides, new_offset = list(self.strides), self._offset
        for i in axes:
          new_strides[i] = -new_strides[i]
          new_offset += self.strides[i] * (self.shape[i] - 1)
        return self.make(self.shape, new_strides, self.device, self._handle, new_offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_shape = tuple(self.shape[i] + sum(axes[i]) for i in range(self.ndim))
        slices = tuple(slice(axes[i][0], new_shape[i] - axes[i][1]) for i in range(self.ndim))
        #print('new_shape: ', new_shape)
        #print('slices: ', slices)
        padded_arr = full(new_shape, 0, device=self.device)
        padded_arr[slices] = self
        return padded_arr
        ### END YOUR SOLUTION
    
    def dilate(self, axes, dilation_amount):
      # added implementation of dilate
      new_shape = tuple(s * (dilation_amount+1) if i in axes else s for i,s in enumerate(self.shape))
      #print('old shape: ', self.shape)
      #print('new shape: ', new_shape)
      dilated_arr = full(new_shape, 0, device=self.device)
      slices = tuple(slice(None, None, dilation_amount+1) if i in axes else slice(None, None, None) for i in range(self.ndim))
      dilated_arr[slices] = self
      return dilated_arr
    
    def undilate(self, axes, dilation_amount):
      # added implementation of undilate
      slices = tuple(slice(None, None, dilation_amount+1) if i in axes else slice(None, None, None) for i in range(self.ndim))
      return self[slices]
...
def stack(args, axis): # added
    return NDArray.stack(args, axis)

def split(a, axis): # added
  return a.split(axis)

def dilate(a, axes, dilation_amount): # added
    return a.dilate(axes, dilation_amount)

def undilate(a, axes, dilation_amount): # added
  return a.undilate(a, axes, dilation_amount)
